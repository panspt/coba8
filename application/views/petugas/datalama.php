                                ...
                                <tr id="hid<?= $t->kode_sewa; ?>">
                                    ...
                                    <td class="text-left">
                                        <h6 style="color: red; margin-bottom: -1px; font-size: 17px;">
                                            <?= $timestampg; ?>
                                            <span id="sewa<?= $t->id_sewaps ?>"></span> Jam
                                        </h6>
                                    </td>
                ...
                <div class="row" id="sewabulanan" hidden>
                    ...
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <input type="text" id="hargamanual" class="form-control form-control-lg text-dark text-end numbers" placeholder="Rp. 0" readonly>
                    ...
                    <div class="modal-content  " style="border: 3px solid red;">
                        ...
                        <div class="modal-body ">
                            <table style="width: 100%; " class="text-dark">
                                ...
                                <?php $dibayar = (($t->dibayar >= $t->total) ? $t->total : $t->sisa); ?>
                                <tr>
                                    <td>DIBAYAR</td>
                                    <td width="3%">:</td>
                                ...
                                <?php
                                $siss = (($t->dibayar >= $t->total) ? 0 : $t->sisa)
                                ?>
                                <tr>
                                    <td>SISA</td>
                                    <td width="3%">:</td>
                        ...
                        <table class="table tab" style="font-size: 13px; ">
                            ...
                            <tbody id="kdsw">
                                ...
                                <?php foreach ($kdsw as $d) {
                                    $ke = $this->db->query("SELECT * FROM tb_kembalikanps WHERE kdsewaps='$d->kode_sewa' ")->row();

                                ?>
                                    <tr id="hid2<?= $d->kode_sewa; ?>">
                                        <td class="text-center"><?= $no++; ?></td>
                                        <td end><?= $d->kode_sewa; ?></td>
                                        <td end><?= $d->idmember; ?></td>
                                        <td><?= $d->nama_maktif; ?></td>
                                        <td><?= $d->jenis_ps; ?></td>
                                        <td><?= $d->jml_hari; ?> Hari</td>
                                        <td class="text-center"><?= date('d-m-Y H.i', strtotime($ke->tgl_dikembalikan)); ?></td>
                                        <td class="text-end"><?= number_format($d->total); ?></td>
                                        <td class="text-end"><?= number_format($d->dibayar + $ke->dibayar); ?></td>
                                        <td class="text-end"><?= number_format($ke->sisa); ?></td>
                                        <td class="text-center">
                                            <button data-kd="<?= $d->kode_sewa; ?>" class="btn btn-sm btn-success detail_data2" data-toggle="tooltip" title="Detail Data"><i class="fa fa-folder"></i></button>
            ...
            function simpan_tambah_sewa(id, kd, byr, hrg, hr, tgl, sts, sisa) {
                ...
                if (hrg == '') {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            if (sts == 'simapan') {
                                alert('Data Tambah waktu sewa berhasil disimpan.....');
                                // alert(data)
                                document.location.href = "<?= base_url('petugas/sewa'); ?>";
                            } else if (sts == 'cetak') {
                                print(data);
